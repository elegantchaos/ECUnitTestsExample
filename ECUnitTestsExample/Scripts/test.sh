...
cd ECUnitTests
xcodebuild -project "ECUnitTests.xcodeproj" -target "ECUnitTestIOS" -arch i386 -sdk "iphonesimulator" clean build OBJROOT="$obj" SYMROOT="$sym"
cd ../ECUnitTestsExample
xcodebuild -project "ECUnitTestsExample.xcodeproj" -target "iOS Tests" -arch i386 -sdk "iphonesimulator" clean build OBJROOT="$obj" SYMROOT="$sym" TEST_AFTER_BUILD=YES
